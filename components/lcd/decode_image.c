...
static char *dwork = NULL;
esp_err_t decode_jpg(uint16_t ***pixels, char *data, int width, int height)
{
    int r;
    JDEC decoder;
    JpegDev jd;
    ...
    if (*pixels == NULL || dwork == NULL) {
        ESP_LOGE(TAG, "decoder not init yet\n");
        return ESP_FAIL;
    }

    //Populate fields of the JpegDev struct.
    jd.inData = (const unsigned char *)data;
    jd.inPos = 0;
    jd.outData = *pixels;
    jd.outW = width;
    jd.outH = height;
    ...
    //Prepare and decode the jpeg.
    r = jd_prepare(&decoder, infunc, dwork, WORKSZ, (void *)&jd);
    if (r != JDR_OK) {
        ESP_LOGE(TAG, "Image decoder: jd_prepare failed (%d)", r);
        ret = ESP_ERR_NOT_SUPPORTED;
    }
...
esp_err_t decoder_init(uint16_t ***pixels, int width, int height)
{
    esp_err_t ret = ESP_OK;

    if (pixels == NULL) {
        ESP_LOGE(TAG, "input null\n");
        return ESP_FAIL;
    }
